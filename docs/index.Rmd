...
sif.005 <- sif.005 %>% 
  dplyr::mutate(
    point_in_pol = as.logical(sp::point.in.polygon(point.x = Lon, 
                                               point.y = Lat,
                                               pol.x = pol.br[,1],
                                               pol.y = pol.br[,2])
                              |
                                sp::point.in.polygon(point.x = Lon, 
                                                 point.y = Lat,
                                                 pol.x = pol_NE[,1],
                                                 pol.y = pol_NE[,2])  
                                )
...
sif.005 %>% 
  ggplot2::ggplot(ggplot2::aes(y=Sif))+
  ggplot2::geom_boxplot(fill="dark green",outlier.colour = "black")+
